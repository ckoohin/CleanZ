...
> Base URL: `/workers` — Tất cả endpoint yêu cầu `Authorization: Bearer <token>`
...
```
User đăng ký → Tạo hồ sơ (apply) → Điền thông tin + upload giấy tờ → Admin duyệt/từ chối → Worker hoạt động
```
...
|---|--------|----------|-------|
| 1 | `POST` | `/:userId/apply` | Tạo hồ sơ worker |
...
| 11 | `PATCH` | `/:id/approve` | Duyệt worker *(Admin)* |
| 12 | `PATCH` | `/:id/reject` | Từ chối worker *(Admin)* |

---

## 1. POST `/:userId/apply`

Tạo hồ sơ đăng ký worker cho user.
...
Cập nhật thông tin hồ sơ worker (phone, skills, experience, bio).
...
```http
PATCH /workers/a1b2c3d4-e5f6-7890-abcd-ef1234567890
...
```http
PATCH /workers/a1b2c3d4-e5f6-7890-abcd-ef1234567890/avatar
Authorization: Bearer <token>
Content-Type: multipart/form-data
```
...
**Quyền:** Chủ hồ sơ hoặc Admin.  
**Lưu ý:** **Không thể** upload khi worker đã được `approved`.  
**Content-Type:** `multipart/form-data`  
...
```http
PATCH /workers/a1b2c3d4-e5f6-7890-abcd-ef1234567890/documents
Authorization: Bearer <token>
Content-Type: multipart/form-data
```
...
| `403` | Không có quyền HOẶC worker đã được approved |
| `404` | Không tìm thấy worker |
...
Lấy hồ sơ worker của chính mình (theo token).
...
| Status | Mô tả |
|--------|-------|
| `404` | Không tìm thấy hồ sơ worker |
...
Xem hồ sơ worker theo worker profile ID.

**Quyền:**
- **Admin** — xem bất kỳ worker nào (mọi trạng thái)
- **Chủ hồ sơ** — xem hồ sơ của chính mình
- **Người khác** — chỉ xem được worker đã `approved`

### Request

```http
GET /workers/a1b2c3d4-e5f6-7890-abcd-ef1234567890
Authorization: Bearer <token>
```
...
| Status | Mô tả |
|--------|-------|
| `404` | Không tìm thấy worker (hoặc chưa approved đối với user thường) |

---

## 7. GET `/:id/documents`

Lấy danh sách tất cả giấy tờ của worker.
...
```http
GET /workers/a1b2c3d4-e5f6-7890-abcd-ef1234567890/documents
Authorization: Bearer <token>
```
...
```http
GET /workers/a1b2c3d4-e5f6-7890-abcd-ef1234567890/documents/citizenCard
Authorization: Bearer <token>
```
...
| `403` | Không có quyền |
| `404` | Không tìm thấy worker HOẶC chưa có ảnh loại này |

---

## 9. GET `/presence/me`

Lấy trạng thái hoạt động hiện tại của worker đang đăng nhập.

**Quyền:** Chỉ user có role `WORKER` và đã được `approved`.

### Request

```http
GET /workers/presence/me
Authorization: Bearer <token>
```
...
| Status | Mô tả |
|--------|-------|
| `403` | Không phải role `WORKER` HOẶC chưa được approved |
| `404` | Không tìm thấy hồ sơ worker |
...
Phê duyệt hồ sơ worker. Tự động:
- `approvalStatus` → `approved`
- Role user → `WORKER`
- Tạo `WorkerPresence` (OFFLINE) nếu chưa có
- Gửi email thông báo
...
```http
PATCH /workers/a1b2c3d4-e5f6-7890-abcd-ef1234567890/approve
...
| Status | Mô tả |
|--------|-------|
| `400` | Worker đã được phê duyệt trước đó |
...
Từ chối hồ sơ worker. Gửi email thông báo từ chối.
...
```http
PATCH /workers/a1b2c3d4-e5f6-7890-abcd-ef1234567890/reject
Authorization: Bearer <admin_token>
```
...
| Status | Mô tả |
|--------|-------|
| `400` | Worker đã bị từ chối trước đó |
| `403` | Không phải Admin |
| `404` | Không tìm thấy worker |
...
### Worker — Đăng ký & hoàn thiện hồ sơ

| # | Method | Endpoint | Quyền | Mô tả |
|---|--------|----------|-------|-------|
| 1 | `POST` | `/:userId/apply` | User/Admin | Tạo hồ sơ worker |
...
> \* User thường chỉ xem được worker đã `approved`

### Worker — Trạng thái hoạt động (sau khi approved)

| # | Method | Endpoint | Quyền | Mô tả |
|---|--------|----------|-------|-------|
| 9 | `GET` | `/presence/me` | Worker (approved) | Xem trạng thái |
| 10 | `PATCH` | `/presence/me` | Worker (approved) | Cập nhật trạng thái |

### Admin — Duyệt hồ sơ

| # | Method | Endpoint | Quyền | Mô tả |
|---|--------|----------|-------|-------|
| 11 | `PATCH` | `/:id/approve` | **Admin** | Phê duyệt worker |
| 12 | `PATCH` | `/:id/reject` | **Admin** | Từ chối worker |
