...
import { exclusiveEndOfDay } from 'src/common/helpers/earnings-period.helper';
...
export class TaskerService {
  ...
  constructor(
    ...
    private readonly taskerEarningsQuery: TaskerEarningsQueryService,
  ) {}
   ...
   *
   * Phép tính nằm ở `TaskerEarningsQueryService` để bảng kê PDF gửi qua email và
   * API này dùng chung một nguồn duy nhất. Shape trả về giữ nguyên như trước để
   * không phá contract của client hiện tại.
   */
  ...
  > {
    return asyncHandleOperation(async () => {
      ...
      const rows = await this.taskerEarningsQuery.findCompletedBookings(
        id,
        fromDate && toDate
          ? { from: fromDate, to: exclusiveEndOfDay(toDate) }
          : undefined,
      );

      // Service trả theo `completedAt` tăng dần; API này vốn trả giảm dần.
      return rows
        .slice()
        .reverse()
        .map((row) => ({
          ...
          taskerEarning: row.taskerEarning,
