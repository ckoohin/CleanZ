...
export class AddMissingDomainTables1782020000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DO $$ BEGIN CREATE TYPE "public"."vouchers_type_enum" AS ENUM('PERCENT', 'FIXED'); EXCEPTION WHEN duplicate_object THEN NULL; END $$`,
    );
    await queryRunner.query(
      `DO $$ BEGIN CREATE TYPE "public"."tasker_withdrawal_requests_status_enum" AS ENUM('PENDING', 'APPROVED', 'REJECTED', 'PROCESSED'); EXCEPTION WHEN duplicate_object THEN NULL; END $$`,
    );
    await queryRunner.query(
      `DO $$ BEGIN CREATE TYPE "public"."wallets_owner_type_enum" AS ENUM('CUSTOMER', 'TASKER', 'SYSTEM'); EXCEPTION WHEN duplicate_object THEN NULL; END $$`,
    );
    await queryRunner.query(
      `DO $$ BEGIN CREATE TYPE "public"."wallet_transaction_type" AS ENUM('DEPOSIT', 'WITHDRAW', 'PAYMENT', 'REFUND', 'PLATFORM_FEE', 'TASKER_EARNING', 'DEPOSIT_HOLD', 'DEPOSIT_RELEASE', 'DEPOSIT_DEDUCT', 'CANCELLATION_FEE', 'ADJUSTMENT'); EXCEPTION WHEN duplicate_object THEN NULL; END $$`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP TABLE IF EXISTS "tasker_withdrawal_requests"`,
    );
    ...
    await queryRunner.query(
      `DROP TYPE IF EXISTS "public"."wallets_owner_type_enum"`,
    );
    await queryRunner.query(
      `DROP TYPE IF EXISTS "public"."tasker_withdrawal_requests_status_enum"`,
    );
    await queryRunner.query(
      `DROP TYPE IF EXISTS "public"."vouchers_type_enum"`,
    );
  }
}
