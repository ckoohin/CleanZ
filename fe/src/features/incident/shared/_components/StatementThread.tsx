...
> = {
  TASKER: { label: "Tasker", icon: Wrench, cls: "bg-[#F59E0B]/15 text-[#B45309]" },
  CUSTOMER: { label: "Khách hàng", icon: User, cls: "bg-[#3B82F6]/15 text-[#1D4ED8]" },
...
export function StatementThread({ statements }: { statements: Statement[] }) {
  ...
  return (
    <div className="space-y-2.5">
      {statements.map((s) => {
        const meta = (s.submittedByRole && ROLE_META[s.submittedByRole]) || null;
        ...
        return (
          <div key={s.id} className="rounded-xl border border-border/40 bg-muted/30 p-3">
            <div className="mb-1.5 flex items-center justify-between gap-2">
              ...
              <span className="text-[10px] text-muted-foreground">{fmt(s.createdAt)}</span>
            </div>
