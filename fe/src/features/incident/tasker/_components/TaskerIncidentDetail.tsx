...
import { MyResponseHistory } from "./MyResponseHistory";
import {
  IncidentStatusBadge,
  SeverityBadge,
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      ...
      <div className="mx-auto max-w-lg space-y-5 p-4">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {inc.statementDueAt && (
            <span className="flex items-center gap-1 text-xs text-amber-600">
              <Clock className="size-3.5" /> Hạn gửi ý kiến:{" "}
              ...
              <div className="mt-1 flex flex-wrap gap-x-3 gap-y-0.5 text-xs text-muted-foreground">
                ...
                {it.approvedAmount != null && (
                  <span className="text-emerald-600">
                    Duyệt: <b>{formatVnd(it.approvedAmount)}</b>
                  </span>
                )}
              </div>
            </div>
        ...
        {/* Kết luận + CĂN CỨ. Phải nằm TRƯỚC ô nhập ý kiến: đọc rồi mới phản biện. */}
        <DecisionBrief incident={inc} />
          ...
          {(inc.myOutstandingDebt ?? 0) > 0 && (
            <div>
              ...
              <p className="font-semibold text-amber-600">
                {formatVnd(inc.myOutstandingDebt)}
              </p>
            </div>
          )}
        </div>
        {(inc.myOutstandingDebt ?? 0) > 0 && (
          <p className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-3 text-xs leading-snug text-amber-700 dark:text-amber-400">
            CleanZ đã trả cho khách thay bạn{" "}
            <b>{formatVnd(inc.myOutstandingDebt)}</b>. Khoản này sẽ được trừ dần
            vào thu nhập các đơn sau, và bạn tạm thời chưa rút tiền được cho tới
            khi trả xong.
          </p>
        )}
        ...
        <div className="space-y-2">
          ...
          <MyResponseHistory
            responses={inc.myDecisionResponses ?? []}
            currentVersion={inc.decisionVersion}
          />
          <StatementThread statements={inc.statements} />
          <StatementComposer
            incidentId={incidentId}
            canSubmit={inc.canSubmitStatement}
          />
        </div>
      </div>
    </div>
