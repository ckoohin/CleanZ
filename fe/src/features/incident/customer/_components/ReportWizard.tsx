...
import {
  CLAIM_MAX,
  REPORT_WINDOW_HOURS_FALLBACK,
  REPORT_WINDOW_SEVERE_HOURS_FALLBACK,
...
export function ReportWizard({
  ...
  /**
   * Nhãn đơn để hiển thị. Khi vào từ link `?bookingId=` thì chưa có, để trống và chỉ hiện
   * ô đã khoá — không bịa ra mã đơn mà mình không biết.
   */
    ...
    createIncident.mutate(
      ...
      },
      {
        ...
        onError: (err: unknown) => {
          ...
          const e = err as {
            response?: { status?: number; data?: { incidentId?: string } };
          };
  ...
  return (
    <div className="min-h-screen bg-background pb-28">
      ...
      <div className="mx-auto max-w-lg space-y-5 p-4">
        <div className="flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-500/20 dark:bg-amber-500/10">
          ...
          <p className="text-xs text-amber-700 dark:text-amber-400">
            Chỉ báo cáo được cho đơn <b>đã hoàn thành</b> và còn trong thời hạn.
            Hãy chụp ảnh cho từng khoản thiệt hại để được xử lý nhanh hơn.
          </p>
        </div>

        {/* Đơn liên quan — CHỌN, không gõ. Mã đơn là UUID; bắt khách tự nhập nó là
            khoá luôn tính năng với mọi người không đi từ link có sẵn. */}
        <div className="space-y-1.5">
          <Label className="text-xs font-semibold">
            Đơn dịch vụ xảy ra sự cố *
          </Label>
          {bookingId ? (
            <div className="flex items-center gap-2 rounded-lg border border-border/60 bg-muted/40 px-3 py-2.5">
              ...
              <span className="min-w-0 flex-1 truncate text-sm font-medium">
                {bookingLabel ?? "Đơn đã chọn"}
              </span>
              {/* Vào từ link của một đơn cụ thể thì không cho đổi: sự cố phải gắn đúng
                  đơn khách vừa bấm, đổi ở đây chỉ tạo cơ hội gắn nhầm. */}
              {!initialBookingId && (
                <button
                  type="button"
                  onClick={() => {
            ...
            <BookingPicker
              reportWindowHours={
                reportConfig?.reportWindowHours ?? REPORT_WINDOW_HOURS_FALLBACK
              }
              reportWindowSevereHours={
                reportConfig?.reportWindowSevereHours ??
                REPORT_WINDOW_SEVERE_HOURS_FALLBACK
              }
              onPick={(b) => {
                setBookingId(b.id);
                setBookingLabel(
                  `${b.bookingCode} · ${b.service?.name ?? "Đơn dịch vụ"}`,
                );
              }}
            />
          )}
        ...
        {/* Chưa chọn đơn thì chưa khai gì cả. Mở sẵn cả form bên dưới một danh sách đơn
            chỉ khiến khách khai xong rồi mới nhận ra còn thiếu bước đầu tiên. */}
        {!bookingId ? null : (
          <>
            <div className="space-y-1.5">
            ...
            <div className="space-y-3">
              ...
              {overClaimMax && (
                <p className="text-xs text-red-500">
                  Tổng số tiền yêu cầu vượt mức tối đa {formatVnd(claimMax)}.
                  Hãy giảm bớt hoặc tách ra thành báo cáo khác.
                </p>
              )}
              ...
              {items.map((it, idx) => (
                ...
                >
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setItem(idx, { claimedAmount: e.target.value })
                    }
                  ...
                  {attempted && !itemValid(it) && (
                    <p className="text-xs text-red-500">
                      Cần mô tả, số tiền chẵn (tối đa {formatVnd(claimMax)}) và
                      ít nhất 1 ảnh.
                    </p>
                  )}
                </div>
      ...
      {/* Submit bar — chỉ xuất hiện khi đã có đơn để gắn sự cố vào. */}
      {bookingId && (
