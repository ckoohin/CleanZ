...
interface StepIdentityVerificationProps {
  ...
  onBack: () => void;
  onNext: (data: { citizenCard: File[]; idWithSelfie: File[]; docIdNumber: string }) => void;
...
export const StepIdentityVerification: React.FC<StepIdentityVerificationProps> = ({ 
  ...
  const [citizenCardFiles, setCitizenCardFiles] = useState<File[]>(initialCitizenCardFiles);
  ...
  const handleNext = () => {
    if (!docIdNumber || docIdNumber.length !== 12 || !/^\d+$/.test(docIdNumber)) {
    ...
    <Card className="border-none shadow-none bg-transparent md:shadow-xl md:bg-card/50 md:backdrop-blur-md rounded-none md:rounded-[2.5rem] overflow-hidden">
      ...
      <CardContent className="p-4 md:p-10">
        ...
        <div className="mb-8 space-y-4 max-w-md">
          <Label htmlFor="docIdNumber" className="text-base md:text-lg font-bold">Số CCCD (12 số) <span className="text-destructive">*</span></Label>
          <Input 
            id="docIdNumber"
            type="text" 
            placeholder="Ví dụ: 001203000123" 
            value={docIdNumber}
            onChange={(e) => setDocIdNumber(e.target.value.replace(/\D/g, '').slice(0, 12))}
          ...
          <div className="p-4 md:p-5 bg-card border border-border/50 shadow-sm rounded-2xl md:rounded-[1.5rem] transition-all hover:border-primary/30 hover:shadow-md">
            ...
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5 pr-2">Tải lên rõ nét 2 mặt trước và sau.</p>
                </div>
              </div>
            ...
            <div 
              ...
              className={cn(
                ...
                totalCCCD < 2 ? "border-primary/40 bg-primary/5 hover:bg-primary/10 hover:border-primary" : "border-muted bg-muted/10 opacity-50 cursor-not-allowed"
              )}
            >
            ...
            {(existingCitizenCards.length > 0 || citizenCardFiles.length > 0) && (
              <div className="mt-3 pt-3 border-t border-border/40 flex flex-wrap gap-2">
                ...
                {existingCitizenCards.map((url, index) => (
                  <div key={`existing-${index}`} className="relative group w-14 h-14 md:w-16 md:h-16 rounded-xl overflow-hidden border border-border shadow-sm">
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm">
                      <Button type="button" variant="destructive" size="icon" onClick={(e) => { e.stopPropagation(); removeExistingFile(url, 'cccd'); }} className="h-6 w-6 rounded-full scale-75 md:scale-100">
                ...
                {citizenCardFiles.map((file, index) => (
                  <div key={`new-${index}`} className="relative group w-14 h-14 md:w-16 md:h-16 rounded-xl overflow-hidden border border-primary/30 shadow-sm">
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm">
                      <Button type="button" variant="destructive" size="icon" onClick={(e) => { e.stopPropagation(); removeFile(index, 'cccd'); }} className="h-6 w-6 rounded-full scale-75 md:scale-100">
                        <X className="w-3 h-3 md:w-4 md:h-4" />
                      </Button>
                    </div>
                  </div>
          ...
          <div className="p-4 md:p-5 bg-card border border-border/50 shadow-sm rounded-2xl md:rounded-[1.5rem] transition-all hover:border-primary/30 hover:shadow-md">
            ...
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  <h4 className="font-bold text-sm md:text-base flex items-center gap-1.5">
                    2. Ảnh Selfie cầm CCCD <span className="text-destructive">*</span>
                  </h4>
                  <p className="text-xs text-muted-foreground mt-0.5 pr-2">Chụp rõ khuôn mặt bạn và mặt trước CCCD.</p>
                </div>
              </div>
              <div className="bg-primary/10 text-primary px-2.5 py-1 rounded-full text-xs font-bold whitespace-nowrap shrink-0">
                {totalSelfie}/1 ảnh
            ...
            <div 
              ...
              className={cn(
                "border-2 border-dashed rounded-[1rem] p-3 md:p-4 flex flex-col md:flex-row items-center justify-center gap-2 md:gap-3 cursor-pointer transition-all duration-300",
                totalSelfie < 1 ? "border-primary/40 bg-primary/5 hover:bg-primary/10 hover:border-primary" : "border-muted bg-muted/10 opacity-50 cursor-not-allowed"
              )}
            >
              <Upload className="w-5 h-5 text-primary" />
              <span className="text-sm font-semibold text-primary">Nhấn để chọn ảnh tải lên</span>
            ...
            {(existingSelfies.length > 0 || selfieFiles.length > 0) && (
              <div className="mt-3 pt-3 border-t border-border/40 flex flex-wrap gap-2">
                ...
                {existingSelfies.map((url, index) => (
                  <div key={`existing-selfie-${index}`} className="relative group w-14 h-14 md:w-16 md:h-16 rounded-xl overflow-hidden border border-border shadow-sm">
                    <img src={url} alt="Selfie cũ" className="w-full h-full object-cover" />
                    <div className="absolute inset-x-0 bottom-0 bg-emerald-500/90 text-[8px] text-white text-center py-0.5 font-bold">
                      Đã tải
                    </div>
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm">
                      <Button type="button" variant="destructive" size="icon" onClick={(e) => { e.stopPropagation(); removeExistingFile(url, 'selfie'); }} className="h-6 w-6 rounded-full scale-75 md:scale-100">
                        <X className="w-3 h-3 md:w-4 md:h-4" />
                      </Button>
                    </div>
                  </div>
                ...
                {selfieFiles.map((file, index) => (
                  <div key={`new-selfie-${index}`} className="relative group w-14 h-14 md:w-16 md:h-16 rounded-xl overflow-hidden border border-primary/30 shadow-sm">
                    <img src={URL.createObjectURL(file)} alt="Selfie mới" className="w-full h-full object-cover" />
                    <div className="absolute inset-x-0 bottom-0 bg-primary/90 text-[8px] text-white text-center py-0.5 font-bold">
                      Mới chọn
                    </div>
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm">
                      <Button type="button" variant="destructive" size="icon" onClick={(e) => { e.stopPropagation(); removeFile(index, 'selfie'); }} className="h-6 w-6 rounded-full scale-75 md:scale-100">
                        <X className="w-3 h-3 md:w-4 md:h-4" />
                      </Button>
                    </div>
                  </div>
