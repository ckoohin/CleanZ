...
export const StepLegalAndPayment: React.FC<StepLegalAndPaymentProps> = ({
  ...
  const [existingCriminalRecord, setExistingCriminalRecord] = useState<
    string[]
  >([]);
  ...
  React.useEffect(() => {
    if (initialDocuments && initialDocuments.length > 0) {
      const crDocs = initialDocuments
        .filter((d) => d.type === "criminalRecord")
        .map((d) => d.fileUrl);
      const hcDocs = initialDocuments
        .filter((d) => d.type === "healthCertificate")
        .map((d) => d.fileUrl);
      const certDocs = initialDocuments
  ...
  const handleFileChange = (
    ...
    if (e.target.files) {
      ...
      if (currentFiles.length + existingCount + newFiles.length > maxFiles) {
        toast.error(
          `Chỉ được upload tối đa ${maxFiles} ảnh cho ${docName}. Hãy xóa bớt ảnh cũ hoặc ảnh mới để tải lại.`,
        );
  ...
  const removeExistingFile = (
    url: string,
    type: "criminalRecord" | "healthCertificate" | "certificate",
  ) => {
      ...
      <div className="p-4 md:p-5 bg-card border border-border/50 shadow-sm rounded-2xl md:rounded-[1.5rem] transition-all hover:border-primary/30 hover:shadow-md">
        ...
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-start gap-2.5">
            <div
              className={cn(
                "p-1.5 md:p-2 rounded-lg bg-opacity-10 mt-0.5 shrink-0",
              ...
              >
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm">
                  <Button
                    ...
                    variant="destructive"
                    size="icon"
                    className="w-6 h-6 rounded-full scale-75 md:scale-100"
                    onClick={(e) => {
                      e.stopPropagation();
                      removeExisting(url);
                    }}
    ...
    <Card className="border-none shadow-none bg-transparent md:shadow-xl md:bg-card/50 md:backdrop-blur-md rounded-none md:rounded-[2.5rem] overflow-hidden">
      <CardHeader className="pt-6 px-4 md:pt-10 md:px-10">
        ...
        <CardDescription className="text-base md:text-lg">
          Hoàn thiện hồ sơ pháp lý và tài khoản nhận lương để bắt đầu công việc.
        </CardDescription>
      </CardHeader>
          ...
          >
            ...
            <div className="bg-primary/5 p-4 md:p-8 rounded-[1.5rem] md:rounded-[2rem] border border-primary/20 space-y-4 md:space-y-6">
              <h3 className="flex items-center gap-2 text-lg font-bold md:text-xl">
                ...
                Thông tin thanh toán (Nhận lương)
              </h3>
              <p className="text-xs md:text-sm text-muted-foreground mb-2 md:mb-4">
                Lưu ý: Tên chủ tài khoản bắt buộc phải khớp với tên trên CCCD.
              </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-sm md:text-base flex items-center gap-2">
                        <CreditCard className="w-4 h-4 text-primary" /> Số Tài
                        Khoản
                      </FormLabel>
                      <FormControl>
                        <Input
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="md:col-span-2">
                      <FormLabel className="text-sm md:text-base flex items-center gap-2">
                        <UserCircle className="w-4 h-4 text-primary" /> Tên Chủ
                        Tài Khoản
                      </FormLabel>
                      <FormControl>
                        <Input
                          placeholder="TÊN KHÔNG DẤU (VD: NGUYEN VAN A)"
                          className="h-11 md:h-12 rounded-xl bg-background uppercase text-sm md:text-base"
                          {...field}
                        />
                      </FormControl>
                      <FormMessage />
