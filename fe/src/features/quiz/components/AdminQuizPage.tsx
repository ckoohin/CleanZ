...
function QuizModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-lg bg-[var(--c-card)] rounded-2xl shadow-xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--c-line)]">
          ...
          <button onClick={onClose} className="text-[var(--c-muted)] hover:text-[var(--c-ink)]"><X className="w-5 h-5" /></button>
        </div>

        <div className="p-6 space-y-4">
          <div>
            <label className="block text-sm font-medium text-[var(--c-ink-soft)] mb-1.5">Tên bài kiểm tra <span className="text-rose-500">*</span></label>
            <input
              type="text"
              value={title}
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--c-ink-soft)] mb-1.5">Mô tả</label>
            <textarea
              rows={2}
              value={description}
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-medium text-[var(--c-ink-soft)] mb-1.5">
                Thời gian (phút)
                <span className="text-xs font-normal text-[var(--c-muted)] block">-1 = không giới hạn</span>
              </label>
              <input
                type="number"
                value={timeLimit}
        ...
        <div className="flex justify-end gap-3 px-6 py-4 border-t border-[var(--c-line)]">
          <button onClick={onClose} className="px-4 py-2 text-sm text-[var(--c-muted)] hover:text-[var(--c-ink)] transition-colors">Hủy</button>
          <button
...
function QuizDetailPanel({ quiz, onClose }: { quiz: AdminQuiz; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex">
      ...
      <div className="w-full max-w-2xl bg-[var(--c-card)] shadow-2xl flex flex-col overflow-hidden">
        {/* Header */}
        <div className="flex items-start justify-between px-6 py-4 border-b border-[var(--c-line)]">
          <div>
            <h2 className="font-semibold text-[var(--c-ink)]">{quiz.title}</h2>
          ...
          <div className="flex items-center gap-2">
            ...
            {quiz.isActive && (
              <span className="px-2 py-1 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 text-xs font-medium rounded-lg">Đang hoạt động</span>
            )}
            <button onClick={onClose} className="text-[var(--c-muted)] hover:text-[var(--c-ink)] ml-1"><X className="w-5 h-5" /></button>
          </div>
        </div>
        ...
        <div className="flex border-b border-[var(--c-line)]">
          {(["questions", "attempts", "stats"] as const).map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
              className={`px-5 py-3 text-sm font-medium border-b-2 transition-colors ${tab === t ? "border-[var(--c-primary)] text-[var(--c-primary-strong)]" : "border-transparent text-[var(--c-muted)] hover:text-[var(--c-ink)]"}`}
            >
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {tab === "questions" && (
            <div className="p-5 space-y-5">
              {/* Assigned questions */}
              <div>
                <h3 className="text-sm font-semibold text-[var(--c-ink-soft)] mb-3 flex items-center gap-2">
                  <ListOrdered className="w-4 h-4" />
                  Câu hỏi trong bài ({quizQuestionsQuery.data?.length ?? 0})
                </h3>
                {quizQuestionsQuery.isLoading ? (
                  <div className="flex justify-center py-6"><Loader2 className="w-5 h-5 animate-spin text-[var(--c-primary-strong)]" /></div>
                ) : (quizQuestionsQuery.data ?? []).length === 0 ? (
                  <p className="text-sm text-[var(--c-muted)] py-4 text-center">Chưa có câu hỏi nào được gán</p>
                ) : (
                  <div className="space-y-2">
                    {(quizQuestionsQuery.data ?? []).map((q: any, i: number) => (
                      <div key={q.id} className="flex items-center gap-3 p-3 rounded-xl bg-[var(--c-card-2)]">
                        <span className="text-xs text-[var(--c-muted)] w-5 shrink-0">{i + 1}</span>
                        <p className="flex-1 text-sm text-[var(--c-ink-soft)] line-clamp-1">{q.questionText}</p>
                        <button
                          onClick={() => removeMutation.mutate(q.id)}
                          disabled={removeMutation.isPending}
                          className="shrink-0 text-[var(--c-muted)] hover:text-rose-500 transition-colors disabled:opacity-40"
                        >
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full mb-3 rounded-xl border border-[var(--c-line-strong)] bg-[var(--c-card-2)] px-3 py-2 text-sm text-[var(--c-ink)] focus:outline-none focus:ring-2 focus:ring-[var(--c-primary)]/40"
                />
                {bankItems.length > 0 && (
                ...
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {bankItems.map((q) => (
                    <label
                      key={q.id}
                      className={`flex items-start gap-3 p-3 rounded-xl cursor-pointer transition-colors ${selectedIds.has(q.id) ? "bg-[var(--c-primary-soft)]" : "hover:bg-[var(--c-card-2)]"}`}
                    >
                      <input
                        type="checkbox"
                        checked={selectedIds.has(q.id)}
                        onChange={() => toggleSelect(q.id)}
                        className="mt-0.5 accent-[var(--c-primary)] w-4 h-4 shrink-0"
                      />
                      <p className="text-sm text-[var(--c-ink-soft)] line-clamp-2">{q.questionText}</p>
                    </label>
                  ))}
                  {bankItems.length === 0 && !bankQuery.isLoading && (
                    <p className="text-sm text-[var(--c-muted)] py-4 text-center">Không có câu hỏi nào phù hợp</p>
                  )}
                </div>
                {selectedIds.size > 0 && (
                  <button
                    ...
                    className="mt-3 flex items-center gap-2 px-4 py-2.5 bg-[var(--c-primary)] hover:bg-[var(--c-primary-strong)] disabled:opacity-60 text-white text-sm font-medium rounded-xl transition-colors"
                  >
          ...
          {tab === "attempts" && (
            <div className="p-5">
              {attemptsQuery.isLoading ? (
                <div className="flex justify-center py-10"><Loader2 className="w-6 h-6 animate-spin text-[var(--c-primary-strong)]" /></div>
              ) : (attemptsQuery.data?.items ?? []).length === 0 ? (
                <p className="text-center text-[var(--c-muted)] py-10">Chưa có lượt thi nào</p>
              ) : (
                <div className="space-y-2">
                  {(attemptsQuery.data?.items ?? []).map((a) => (
                    <div key={a.id} className="flex items-center justify-between p-3 rounded-xl bg-[var(--c-card-2)] text-sm">
                      <div>
                        <p className="font-medium text-[var(--c-ink-soft)]">
                      ...
                      <div className="text-right">
                        <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${a.status === "PASSED" ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400" : a.status === "FAILED" ? "bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-400" : "bg-[var(--c-chip)] text-[var(--c-muted)]"}`}>
                          {a.status === "PASSED" ? "Đạt" : a.status === "FAILED" ? "Không đạt" : a.status === "EXPIRED" ? "Hết giờ" : "Đang thi"}
                        </span>
                        {a.score !== null && <p className="text-xs text-[var(--c-muted)] mt-0.5">{a.score}/100</p>}
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {[
                    ...
                    <div key={s.label} className={`rounded-xl p-4 bg-${s.color}-50 dark:bg-${s.color}-900/20`}>
                      ...
                      <p className="text-xs text-[var(--c-muted)] mt-1">{s.label}</p>
                    </div>
                  ))}
                </div>
              ) : null}
  ...
  return (
    <div className="p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-[var(--c-ink)]">Quản lý bài kiểm tra</h1>
          <p className="text-sm text-[var(--c-muted)] mt-0.5">{quizzes.length} bài kiểm tra</p>
        </div>
        <div className="flex items-center gap-2">
          <button
      ...
      {query.isLoading ? (
        <div className="flex justify-center py-20"><Loader2 className="w-8 h-8 animate-spin text-[var(--c-primary-strong)]" /></div>
      ) : quizzes.length === 0 ? (
        <div className="text-center py-20 text-[var(--c-muted)]">Chưa có bài kiểm tra nào</div>
      ) : (
        <div className="grid gap-4">
          {quizzes.map((quiz) => (
            <div key={quiz.id} className={`rounded-2xl border ${quiz.isActive ? "border-emerald-300 dark:border-emerald-700 bg-emerald-50/30 dark:bg-emerald-900/10" : "border-[var(--c-line)] bg-[var(--c-card)]"} p-5`}>
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <h2 className="font-semibold text-[var(--c-ink)]">{quiz.title}</h2>
                    {quiz.isActive && (
                      <span className="px-2 py-0.5 bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400 text-xs font-medium rounded-full">Đang hoạt động</span>
                    )}
                  </div>
                  {quiz.description && <p className="text-sm text-[var(--c-muted)] mt-1 line-clamp-1">{quiz.description}</p>}

                  <div className="flex flex-wrap gap-4 mt-3 text-sm text-[var(--c-muted)]">
                ...
                <div className="flex items-center gap-2 ml-4 shrink-0">
                  <button
                    ...
                    className="p-2 text-[var(--c-muted)] hover:text-[var(--c-primary-strong)] transition-colors"
                  >
                    <Pencil className="w-4 h-4" />
                  </button>
                  <button
                    onClick={() => setSelected(quiz)}
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-[var(--c-chip)] hover:bg-[var(--c-line)] text-[var(--c-ink-soft)] text-xs font-medium rounded-lg transition-colors"
                  >
