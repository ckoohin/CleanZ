...
const ACTIVE_STATUSES: BookingStatus[] = [
  "POSTED", "CONFIRMED", "TASKER_ON_THE_WAY", "CHECKED_IN", "IN_PROGRESS",
];
const DONE_STATUSES: BookingStatus[] = ["COMPLETED", "CANCELLED", "EXPIRED"];
...
// ─── History (completed/cancelled) Card ───────────────────────────────────────
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="text-right shrink-0">
          <p className="font-black text-sm text-foreground">{fmtCurrency(booking.price.totalPrice)}</p>
...
export const HistoryPage = () => {
  ...
  const { data, isLoading, refetch } = useMyActiveBooking();

  // BE trả `booking: CustomerBookingDetail | null` (active booking)
  // Tạm dùng data.booking cho ACTIVE tab
  // TODO: Thêm API GET /booking/history khi BE sẵn sàng
  const activeBooking = data?.booking ?? null;
          ...
          <AnimatePresence mode="wait">
              ...
              >
                {activeBooking ? (
                  <ActiveBookingCard
                    ...
                    onClick={() => goToDetail(activeBooking.id)}
                  />
                ) : (
            ...
            {activeTab === "HISTORY" && (
              <motion.div
                ...
                className="space-y-3"
              >
                {/* TODO: khi BE có GET /booking/my-bookings?status=COMPLETED,CANCELLED */}
                <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-start gap-3">
                  <AlertTriangle className="w-4 h-4 text-amber-600 shrink-0 mt-0.5" />
                  <div>
                    <p className="text-xs font-bold text-amber-700">API lịch sử đang phát triển</p>
                    <p className="text-xs text-amber-600/80 mt-0.5">
                      BE chưa có endpoint GET /booking/history. Sẽ cập nhật khi sẵn sàng.
                    </p>
                  </div>
                </div>
