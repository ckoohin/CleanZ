...
function ServiceRow({ svc, onViewDetail, onEdit, onDelete, onToggle, isToggling }: {
  ...
  return (
    <div className={cn(
      ...
      expanded ? "border-primary/40 shadow-lg" : "border-border/50 shadow-sm hover:border-primary/20 hover:shadow-md",
    )}>
      {/* Header row */}
      <div className="flex items-center gap-3 p-3.5">
        <div className="relative h-12 w-12 rounded-xl overflow-hidden bg-muted/40 shrink-0 border border-border/40 cursor-pointer"
        ...
        <div className="flex-1 min-w-0 cursor-pointer" onClick={() => setExpanded(!expanded)}>
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="font-bold text-foreground text-sm hover:text-primary transition-colors">{svc.name}</span>
            <span className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-primary/10 text-primary font-bold hidden sm:inline">{svc.subServiceCode}</span>
          ...
          <div className="flex flex-wrap items-center gap-2 mt-0.5 text-xs text-muted-foreground">
            ...
            {bp && <span className="flex items-center gap-0.5 text-primary font-semibold"><DollarSign className="w-3 h-3" aria-hidden="true" />{vnd(bp)}</span>}
            ...
            {(svc.includedTasks?.length ?? 0) > 0 && <span className="flex items-center gap-0.5 text-emerald-600"><Check className="w-3 h-3" aria-hidden="true" />{svc.includedTasks?.length} tác vụ</span>}
          </div>
        </div>

        <div className="flex items-center gap-0.5 shrink-0">
          <button type="button" title={svc.isActive ? "Tắt" : "Bật"} onClick={e => { e.stopPropagation(); onToggle(svc); }} disabled={isToggling}
            className={cn("p-2 rounded-xl transition-colors disabled:opacity-40", svc.isActive ? "text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/20" : "text-muted-foreground hover:bg-muted")}>
          ...
          <button type="button" onClick={() => setExpanded(!expanded)} className="p-2 rounded-xl text-muted-foreground hover:bg-muted transition-colors">
      ...
      {/* Expanded */}
      {expanded && (
        <div className="border-t border-border/50 bg-muted/5 p-4 animate-in fade-in-0 slide-in-from-top-1 duration-200 space-y-4">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
            {[
              { label: "Giá gốc", value: bp ? vnd(bp) : "Chưa thiết lập", color: "text-primary", bg: "bg-primary/10", icon: DollarSign },
              { label: "Giá cao điểm", value: svc.pricingConfig?.peakPrice ? vnd(svc.pricingConfig.peakPrice) : "—", color: "text-amber-500", bg: "bg-amber-50 dark:bg-amber-900/20", icon: TrendingUp },
              { label: "Thời lượng", value: svc.durationHours ? `${svc.durationHours}h` : "—", color: "text-blue-500", bg: "bg-blue-50 dark:bg-blue-900/20", icon: Clock },
              { label: "Tasks bao gồm", value: `${svc.includedTasks?.length ?? 0} mục`, color: "text-emerald-500", bg: "bg-emerald-50 dark:bg-emerald-900/20", icon: Check },
            ].map(s => (
              <div key={s.label} className="bg-card border border-border/40 rounded-xl p-3 flex items-center gap-2">
          ...
          {svc.shortDescription && (
            <p className="text-sm text-muted-foreground bg-background p-3 rounded-xl border border-border/40 leading-relaxed">
              {svc.shortDescription}
            </p>
          )}
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button type="button" onClick={() => onEdit(svc)}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-border/50 text-xs font-semibold text-muted-foreground hover:text-blue-600 hover:border-blue-300 transition-colors">
            ...
            <button type="button" onClick={() => onToggle(svc)} disabled={isToggling}
              className={cn("flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-xs font-semibold transition-colors disabled:opacity-40",
                svc.isActive ? "border-rose-200 text-rose-600 hover:bg-rose-50" : "border-emerald-200 text-emerald-600 hover:bg-emerald-50")}>
              <Power className="w-3.5 h-3.5" aria-hidden="true" />{svc.isActive ? "Tắt dịch vụ" : "Bật dịch vụ"}
            </button>
            <button type="button" onClick={() => onDelete(svc)}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-rose-200 text-xs font-semibold text-destructive hover:bg-destructive/10 transition-colors">
    ...
    <div className="space-y-6 w-full">
        ...
        <div className="flex flex-wrap gap-3 items-center">
          ...
          <Select value={packageFilter} onValueChange={v => { setPackageFilter(v); setPage(1); }}>
            <SelectTrigger className="h-9 w-[170px] rounded-xl text-xs font-semibold">
              <LayoutGrid className="w-3.5 h-3.5 mr-1.5 text-muted-foreground" aria-hidden="true" />
          ...
          <div className="ml-auto text-xs text-muted-foreground">
            <span className="font-bold text-foreground">{filtered.length}</span> / {total} dịch vụ
          </div>
        </div>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-3">
          ...
          <p className="text-sm text-muted-foreground">Đang tải danh sách dịch vụ con...</p>
        </div>
      ) : filtered.length === 0 ? (
        <div className="py-20">
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between gap-4 pt-2">
          <p className="text-xs text-muted-foreground">
            ...
            {" "}· <span className="font-bold text-primary">{total}</span> bản ghi
          </p>
