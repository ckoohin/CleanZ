    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
      <div className="w-full rounded-2xl border border-border bg-muted/50 p-10 flex flex-col items-center gap-5 text-center">
        ...
        <Button asChild size="lg" className="rounded-xl px-10 h-12 font-bold shadow-lg shadow-primary/20">
          <Link href="/staff/onboarding" className="flex items-center gap-2">
...
function PendingReviewView({ staff }: { staff: {
  fullName?: string;
  phone?: string;
  bankName?: string;
        ...
        <div className="border-t border-yellow-500/10 pt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-white dark:bg-card p-4 rounded-xl border border-border space-y-1">
              ...
              <p className="text-sm font-semibold">{staff.fullName || 'Chưa cập nhật'}</p>
            </div>
            <div className="bg-white dark:bg-card p-4 rounded-xl border border-border space-y-1">
              <p className="text-xs text-muted-foreground">Số điện thoại</p>
              <p className="text-sm font-semibold">{staff.phone || 'Chưa cập nhật'}</p>
            </div>
            <div className="bg-white dark:bg-card p-4 rounded-xl border border-border space-y-1">
              <p className="text-xs text-muted-foreground">Tài khoản ngân hàng</p>
              <p className="text-sm font-semibold">
                {staff.bankName ? `${staff.bankName} - ${staff.bankAccountNumber}` : 'Chưa cập nhật'}
              </p>
            </div>
            <div className="bg-white dark:bg-card p-4 rounded-xl border border-border space-y-1">
              <p className="text-xs text-muted-foreground">Giấy tờ đính kèm</p>
              <div className="flex flex-wrap gap-1.5 mt-1">
                {staff.hasCitizenCardImage ? (
...
function ProfileCompletionCard({ staff }: { staff: ReturnType<typeof useStaffProfile>['data'] }) {
  if (!staff) return null;

  const checks = [
    { label: 'Ảnh CCCD', done: !!staff.hasCitizenCardImage },
    { label: 'Ảnh selfie + CCCD', done: !!staff.hasIdWithSelfieImage },
    { label: 'Lý lịch tư pháp', done: !!staff.hasCriminalRecordImage },
    { label: 'Giấy khám sức khoẻ', done: !!staff.hasHealthCertificateImage },
    { label: 'Số điện thoại', done: !!staff.phone },
    { label: 'Địa chỉ hiện tại', done: !!staff.addressCurrent },
    { label: 'Thông tin ngân hàng', done: !!staff.bankName },
  ];
    ...
    <div className="rounded-2xl border border-border bg-card p-5 space-y-4">
      ...
      {pct < 100 && (
        <Button asChild size="sm" variant="outline" className="w-full rounded-xl h-9">
          <Link href="/staff/profile">Cập nhật ngay</Link>
        </Button>
      )}
    </div>
  );
...
function ApprovedDashboard({ staff }: { staff: NonNullable<ReturnType<typeof useStaffProfile>['data']> }) {
  ...
  return (
    <div className="space-y-8">
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <h2 className="text-lg font-bold truncate">Chào mừng, {staff.fullName ?? 'Đối tác'}!</h2>
        ...
        <Button asChild size="sm" className="rounded-xl shrink-0">
          <Link href="/staff/schedule" className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={Briefcase} label="Tổng đơn" value={staff.totalJobs} sub="Đã hoàn thành" color="bg-primary/10 text-primary" />
        <StatCard icon={Star} label="Đánh giá TB" value={staff.avgRating > 0 ? staff.avgRating.toFixed(1) + '★' : '—'} sub="Từ khách hàng" color="bg-yellow-500/10 text-yellow-600" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="rounded-2xl border border-border bg-card p-5 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
              ...
              <div className="flex items-center gap-3 text-muted-foreground">
                ...
                <span className="truncate">{staff.addressCurrent ?? 'Chưa cập nhật'}</span>
              </div>
              <div className="flex items-center gap-3 text-muted-foreground">
                <CreditCard className="w-4 h-4 shrink-0" aria-hidden="true" />
                <span>{staff.bankName ? `${staff.bankName} ••••${staff.bankAccountNumber?.slice(-4)}` : 'Chưa liên kết ngân hàng'}</span>
              </div>
            </div>
            <Button asChild variant="outline" size="sm" className="rounded-xl h-9">
              <Link href="/staff/profile" className="flex items-center gap-2">
        ...
        <div>
          <ProfileCompletionCard staff={staff} />
        </div>
      </div>
    </div>
  );
...
function StaffPage() {
  ...
  const isApproved = staff?.approvalStatus === StaffStatus.APPROVED;
  ...
  const renderContent = () => {
    ...
    if (!staff) return <NoProfileBanner />;

    switch (staff.approvalStatus) {
      case StaffStatus.PENDING:    return <PendingReviewView staff={staff} />;
      case StaffStatus.NEED_INFO:  return (
        <div className="space-y-6">
          <NeedInfoBanner adminNotes={staff.adminNotes} />
          <StaffRegistrationWizard />
        </div>
      );
      case StaffStatus.REJECTED:   return <RejectedBanner adminNotes={staff.adminNotes} />;
      case StaffStatus.APPROVED:   return <ApprovedDashboard staff={staff} />;
