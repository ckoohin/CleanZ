...
export default function StaffProfilePage() {
  ...
  const { register, handleSubmit, formState: { errors, isDirty } } = useForm<FormValues>({
    ...
    values: {
      ...
      skills: staff?.skills ?? '',
      addressResident: staff?.addressResident ?? '',
      addressCurrent: staff?.addressCurrent ?? '',
  ...
  const onSubmit = async (data: FormValues) => {
    if (!staff?.id) return;
    await updateProfile.mutateAsync({ id: staff.id, data });
    setSaved(true);
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <SectionCard title="Thông tin cơ bản" icon={User}>
              <div className="flex items-center gap-5">
                ...
                <div>
                  <p className="font-bold text-lg">{staff?.fullName ?? '—'}</p>
            ...
            <SectionCard title="Giấy tờ đã nộp" icon={FileText}>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <DocBadge done={!!staff?.hasCitizenCardImage} label="Ảnh CCCD" />
                <DocBadge done={!!staff?.hasIdWithSelfieImage} label="Selfie + CCCD" />
                <DocBadge done={!!staff?.hasCriminalRecordImage} label="Lý lịch tư pháp" />
                <DocBadge done={!!staff?.hasHealthCertificateImage} label="Khám sức khoẻ" />
                <DocBadge done={!!staff?.hasCertificateImage} label="Chứng chỉ nghề" />
              </div>
              <Button asChild variant="outline" size="sm" className="rounded-xl h-9 w-fit">
                <Link href="/staff/onboarding">Cập nhật giấy tờ</Link>
              </Button>
            </SectionCard>
